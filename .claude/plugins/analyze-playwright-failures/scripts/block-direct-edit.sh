...
if [[ "$FILE_PATH" == *"playwright/"* ]] && [[ "$FILE_PATH" == *".spec.ts"* || "$FILE_PATH" == *".test.ts"* || "$FILE_PATH" == *"/pages/"* ]]; then
    ...
    echo "" >&2
    echo "When running /analyze-playwright-failures, you MUST delegate edits to subagents:" >&2
    echo "" >&2
    echo "Use Task tool with subagent_type: \"analyze-playwright-failures:e2e-test-fixer\"" >&2
    exit 2
fi
