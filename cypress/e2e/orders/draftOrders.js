...
xdescribe("Draft orders", () => {
  ...
  before(() => {
    ...
    getDefaultChannel()
      ...
      .then(addresses => {
        ...
        createCustomer(`${randomName}@example.com`, randomName, addresses.plAddress, true);
        createShipping({
  ...
  it("should move draft order to orders. TC: SALEOR_2103", { tags: ["@orders", "@allEnv"] }, () => {
    ...
    finalizeDraftOrder(randomName, address).then(draftOrderNumber => {
      cy.visit(urlList.orders);
      cy.contains(ORDERS_SELECTORS.orderRow, draftOrderNumber).should($order => {
        expect($order).to.be.visible;
      });
      cy.visit(urlList.draftOrders);
      cy.contains(DRAFT_ORDERS_LIST_SELECTORS.draftOrderRow, draftOrderNumber).should(
        $draftOrder => {
