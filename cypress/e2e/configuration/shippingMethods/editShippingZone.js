...
import { createShippingZone, getShippingZone } from "../../../support/api/requests/ShippingMethod";
...
describe("As a user I should be able to update and delete shipping zone", () => {
  ...
  it(
    ...
    { tags: ["@shipping", "@allEnv", "@stable"] },
    () => {
      cy.visit(shippingZoneDetailsUrl(shippingZone.id)).deleteElementWithReqAlias(
        "DeleteShippingZone",
      );
  ...
  it(
    ...
    () => {
      ...
      createShippingZone(`${startsWith}Second`, "US", defaultChannel.id, warehouse.id).then(
        shippingZoneResp => {
