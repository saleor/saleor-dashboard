export function expectCorrectProductInformation(productResp, productData) {
  ...
  expectCorrectMetadataInfo(productResp.privateMetadata, productData.metadata.private);
...
export function expectCorrectProductVariantInformation(variantsResp, variantName, prices) {
  expect(variantsResp).to.have.length(1);
  expect(variantsResp[0].sku).to.be.eq(variantName);
  expect(variantsResp[0].channelListings[0].costPrice.amount).to.be.eq(prices.costPrice);
  expect(variantsResp[0].channelListings[0].price.amount).to.be.eq(prices.sellingPrice);
}
...
function expectCorrectGeneralInformation(productResp, generalInfo) {
  ...
  expect(productResp.description, "Check product description").includes(generalInfo.description);
...
function expectCorrectSeoInfo(productResp, seo) {
  ...
  expect(productResp.seoDescription, "Check product seo description").to.be.eq(seo.description);
}
...
function expectCorrectMetadataInfo(metadataResp, expectedMetadata) {
  ...
  expect(metadataResp[0].key, "Check product metadata key").to.be.eq(expectedMetadata.name);
  expect(metadataResp[0].value, "Check product metadata value").to.be.eq(expectedMetadata.value);
}
...
function expectCorrectProductOrgInfo(productResp, productOrganization) {
  ...
  expect(productResp.category.name, "Check category name").to.be.eq(productOrganization.category);
  expect(productResp.collections, "Check length of assigned collections").to.have.length(1);
...
function expectCorrectAttribute(attributes, attribute) {
  ...
  expect(attributes[0].attribute.name, "Check attribute name").to.be.eq(attribute.name);
}
