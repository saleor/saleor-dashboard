...
jest.mock("@dashboard/graphql", () => ({
  ...
  useVoucherCodeBulkDeleteMutation: jest.fn(() => [jest.fn(), { loading: false, called: false }]),
}));
describe("useVoucherCodes", () => {
  it("should return manually generated voucher coded", () => {
    ...
    expect(result.current.voucherCodes).toEqual([
      ...
      { id: "code-id-1", code: "code 1", used: 0 },
      { id: "code-id-2", code: "code 2", used: 0 },
    ]);
  });
  it("should stage server code deletes until save", () => {
    // Arrange
    const { result } = renderHook(() => useVoucherCodes({ id: "1" }));

    // Act
    act(() => {
      result.current.setSelectedVoucherCodesIds(["code 1"]);
    });
    act(() => {
      void result.current.handleDeleteVoucherCodes();
    });

    // Assert
    expect(result.current.pendingRemovedCodeIds).toEqual(["code-id-1"]);
    expect(result.current.voucherCodes).toEqual([{ id: "code-id-2", code: "code 2", used: 0 }]);
  });
  it("should cancel pending delete when re-adding the same code", () => {
    // Arrange
    const { result } = renderHook(() => useVoucherCodes({ id: "1" }));
    ...
    // Assert
    expect(result.current.pendingRemovedCodeIds).toEqual([]);
    expect(result.current.voucherCodes).toEqual([
      { id: "code-id-1", code: "code 1", used: 0 },
      { id: "code-id-2", code: "code 2", used: 0 },
    ]);
  ...
  it("should allow to paginate voucher codes comes from server", () => {
    ...
    // Assert
    expect(result.current.voucherCodes).toEqual([
      { id: "code-id-3", code: "code 3", used: 0 },
      { id: "code-id-4", code: "code 4", used: 0 },
    ]);
