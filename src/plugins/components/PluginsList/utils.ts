...
export const getActiveChannelConfigsCount = (channelConfigurations: PluginConfigurationBaseFragment[]) =>
  channelConfigurations?.filter(({ active }) => !!active).length;

export const getPluginStatusLabel = (channelData: PluginConfigurationBaseFragment): MessageDescriptor =>
...
export const mapPluginsToPills = (channelConfigurations: PluginConfigurationBaseFragment[]): Pill[] =>
