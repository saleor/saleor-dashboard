...
import { GiftCardListQuery, GiftCardListQueryVariables, useGiftCardListQuery } from '@dashboard/graphql';
import useBulkActions, { UseBulkActionsProps } from '@dashboard/hooks/useBulkActions';
...
export const GiftCardsListProvider: React.FC<GiftCardsListProviderProps> = ({ children, params }) => {
  ...
  const { updateListSettings, settings } = useListSettings<GiftCardListColummns>(ListViews.GIFT_CARD_LIST);
  ...
  return <GiftCardsListContext.Provider value={providerValues}>{children}</GiftCardsListContext.Provider>;
};
