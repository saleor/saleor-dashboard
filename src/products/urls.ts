...
export const productAddUrl = (params?: ProductCreateUrlQueryParams) => productAddPath + '?' + stringifyQs(params);
...
export type ProductCreateUrlQueryParams = Dialog<ProductCreateUrlDialog> & SingleAction & ProductCreateUrlProductType;
...
) => productVariantEditPath(encodeURIComponent(productId), encodeURIComponent(variantId)) + '?' + stringifyQs(params);
...
export const productVariantAddPath = (productId: string) => urlJoin(productSection, productId, 'variant/add');
export const productVariantAddUrl = (productId: string, params?: ProductVariantAddUrlQueryParams): string =>
  productVariantAddPath(encodeURIComponent(productId)) + '?' + stringifyQs(params);
...
export const productImageUrl = (productId: string, imageId: string, params?: ProductImageUrlQueryParams) =>
  productImagePath(encodeURIComponent(productId), encodeURIComponent(imageId)) + '?' + stringifyQs(params);
