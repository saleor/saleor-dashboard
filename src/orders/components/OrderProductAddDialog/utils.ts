...
type SetVariantsAction = (data: SearchOrderVariantQuery['search']['edges'][0]['node']['variants']) => void;
...
export function hasAllVariantsSelected(
  ...
  return productVariants.reduce(
    (acc, productVariant) =>
      acc && !!selectedVariantsToProductsMap.find(selectedVariant => selectedVariant.id === productVariant.id),
    true,
  );
}
...
export const onProductAdd = (
  ...
  productsWithAllVariantsSelected[productIndex]
    ? setVariants(
        variants.filter(
          selectedVariant => !product.variants.find(productVariant => productVariant.id === selectedVariant.id),
        ),
      )
    : setVariants([
        ...variants,
        ...product.variants.filter(
          productVariant => !variants.find(selectedVariant => selectedVariant.id === productVariant.id),
        ),
      ]);
