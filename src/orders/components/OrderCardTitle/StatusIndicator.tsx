...
const IconCircle = ({ children, backgroundColor, borderColor }: IconCircleProps) => (
  <Box
    ...
    __border={`1px solid ${borderColor}`}
  >
    {children}
  </Box>
);
...
export const StatusIndicator = ({ status }: StatusIndicatorProps): JSX.Element | null => {
  ...
  const getIcon = (): JSX.Element | null => {
    ...
    switch (status) {
      case FulfillmentStatus.FULFILLED:
        return <PackageIcon color={text} size={ICON_SIZE} strokeWidth={strokeWidth} />;
      case FulfillmentStatus.REFUNDED:
        return <RefundedIcon size={ICON_SIZE} color={text} strokeWidth={strokeWidth} />;
      case FulfillmentStatus.RETURNED:
        return <ReturnedIcon size={ICON_SIZE} color={text} strokeWidth={strokeWidth} />;
      case FulfillmentStatus.CANCELED:
        return <EraserIcon color={text} size={ICON_SIZE} strokeWidth={strokeWidth} />;
      case FulfillmentStatus.REPLACED:
        return <ReplaceIcon color={text} size={ICON_SIZE} strokeWidth={strokeWidth} />;
      case FulfillmentStatus.WAITING_FOR_APPROVAL:
        return <SignatureIcon color={text} size={ICON_SIZE} strokeWidth={strokeWidth} />;
      case FulfillmentStatus.REFUNDED_AND_RETURNED: {
