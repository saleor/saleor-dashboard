...
import { channelUsabilityData, clients, draftOrder } from '@dashboard/orders/fixtures';
...
storiesOf('Orders / Order draft', module)
  ...
  .add('loading', () => (
    <OrderDraftPage {...props} disabled={true} order={undefined} channelUsabilityData={undefined} />
  ))
  .add('without lines', () => <OrderDraftPage {...props} order={{ ...order, lines: [] }} />)
  .add('no user permissions', () => <OrderDraftPage {...props} customPermissions={[]} />)
  .add('with errors', () => <OrderDraftPage {...props} errors={finalizeErrors} />);
