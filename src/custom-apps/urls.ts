...
export const CustomAppUrls = {
  ...
  resolveAppListUrl: (params?: CustomAppListUrlQueryParams) => CustomAppPaths.appListPath + '?' + stringifyQs(params),
  resolveWebhookUrl: (appId: string, id: string, params?: CustomAppDetailsUrlQueryParams) =>
    CustomAppPaths.resolveWebhookPath(encodeURIComponent(appId), encodeURIComponent(id)) + '?' + stringifyQs(params),
