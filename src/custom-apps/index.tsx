...
import { CustomAppDetailsUrlQueryParams, CustomAppListUrlQueryParams, CustomAppPaths } from './urls';
...
const CustomAppDetails: React.FC<CustomAppDetailsProps> = ({ match, token, onTokenClose }) => {
  ...
  return <CustomAppDetailsView id={decodeURIComponent(id)} params={params} token={token} onTokenClose={onTokenClose} />;
};

const CustomAppWebhookCreate: React.FC<RouteComponentProps<any>> = ({ match }) => {
...
const CustomAppWebhookDetails: React.FC<RouteComponentProps<any>> = ({ match }) => {
  const id = match.params.id;
  ...
  return (
    <>
      ...
      <Switch>
        ...
        <Route exact path={CustomAppPaths.appAddPath} render={() => <CustomAppCreateView setToken={setToken} />} />
        <Route
          exact
          path={CustomAppPaths.resolveAppPath(':id')}
          render={props => <CustomAppDetails {...props} token={token} onTokenClose={() => setToken(null)} />}
        />
        <Route exact path={CustomAppPaths.resolveWebhookAddPath(':appId')} component={CustomAppWebhookCreate} />
        <Route exact path={CustomAppPaths.resolveWebhookPath(':appId', ':id')} component={CustomAppWebhookDetails} />
      </Switch>
    </>
  );
};
