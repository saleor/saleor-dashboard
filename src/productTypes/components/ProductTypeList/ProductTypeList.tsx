...
const ProductTypeList: React.FC<ProductTypeListProps> = props => {
  const { disabled, productTypes, onSort, isChecked, selected, sort, toggle, toggleAll, toolbar } = props;
  ...
  return (
    <ResponsiveTable>
      ...
      >
        <TableCellHeader
          direction={sort.sort === ProductTypeListUrlSortField.name ? getArrowDirection(sort.asc) : undefined}
          arrowPosition="right"
          onClick={() => onSort(ProductTypeListUrlSortField.name)}
          className={classes.colName}
        >
          <FormattedMessage id="hHOI7D" defaultMessage="Type Name" description="product type name" />
        </TableCellHeader>
        <TableCellHeader
          direction={sort.sort === ProductTypeListUrlSortField.digital ? getArrowDirection(sort.asc) : undefined}
        ...
        <TableCell className={classes.colTax}>
          <FormattedMessage id="+Jgot0" defaultMessage="Tax class" description="tax class for a product type" />
        </TableCell>
      </TableHead>
      <TableFooter>
        <TableRowLink>
          ...
          productType => {
            ...
            return (
              ...
              >
                ...
                <TableCell className={classes.colType}>
                  {maybe(() => productType.isShippingRequired) !== undefined ? (
                    productType.isShippingRequired ? (
                      <>
                        <FormattedMessage id="ADTNND" defaultMessage="Physical" description="product type" />
                      </>
                    ) : (
                      <>
                        <FormattedMessage id="asdvmK" defaultMessage="Digital" description="product type" />
                      </>
                    )
                  ) : (
                    <Skeleton />
                ...
                <TableCell className={classes.colTax}>{productType?.taxClass?.name ?? '-'}</TableCell>
              </TableRowLink>
            );
          },
          () => (
