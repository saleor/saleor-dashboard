...
export function areAddressInputFieldsModified(data: SiteSettingsPageAddressFormData): boolean {
  return (
    ['city', 'country', 'countryArea', 'phone', 'postalCode', 'streetAddress1', 'streetAddress2'] as Array<
      keyof SiteSettingsPageAddressFormData
    >
  )
    .map(key => data[key])
    .some(field => field !== '');
...
const SiteSettingsPage: React.FC<SiteSettingsPageProps> = props => {
  ...
  const [displayCountry, setDisplayCountry] = useStateFromProps(shop?.companyAddress?.country.country || '');

  const { errors: validationErrors, submit: handleSubmitWithAddress } = useAddressValidation(onSubmit);
      ...
      {({ change, data, set, isSaveDisabled, submit }) => {
        ...
        return (
          <DetailedContent useSingleColumn>
            <TopNav href={configurationMenuUrl} title={intl.formatMessage(commonMessages.generalInformations)} />
            <Content>
              <Grid variant="inverted" className={sprinkles({ paddingLeft: 9 })}>
                <PageSectionHeader
                  title={intl.formatMessage(messages.sectionCheckoutTitle)}
                  description={intl.formatMessage(messages.sectionCheckoutDescription)}
                />
                <SiteCheckoutSettingsCard data={data} errors={errors} disabled={disabled} onChange={change} />
