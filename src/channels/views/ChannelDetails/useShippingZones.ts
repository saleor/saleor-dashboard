...
import { PermissionEnum, useChannelShippingZonesQuery, useShippingZonesCountQuery } from '@dashboard/graphql';
...
export const useShippingZones = (channelId?: string) => {
  ...
  const { data: shippingZonesCountData, loading: shippingZonesCountLoading } = useShippingZonesCountQuery({
    skip: !canLoadShippingZones,
  });

  const { data: channelShippingZonesData, loading: channelsShippingZonesLoading } = useChannelShippingZonesQuery({
    variables: {
      filter: {
        channels: [channelId],
      },
