...
export function isMenuActive(location: string, menuItem: SidebarMenuItem) {
  ...
  if (activeUrl === orderDraftListUrl().split('?')[0] && menuItemUrl === orderListUrl().split('?')[0]) {
    return false;
  }
...
export const getMenuItemExtension = (extensions: Record<AppExtensionMountEnum, Extension[]>, id: string) => {
  const extensionsList = Object.values(extensions).reduce((list, extensions) => list.concat(extensions), []);
