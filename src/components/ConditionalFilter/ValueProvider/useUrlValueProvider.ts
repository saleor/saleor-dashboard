...
export const useUrlValueProvider = (): FilterValueProvider => {
  ...
  const tokenizedUrl = useTokenArray(router.location.search.slice(1))
  const fetchingParams = tokenizedUrl.getFetchingParams()
  const { data, loading } = useInitialAPIState(fetchingParams);
  const value = loading ? [] : tokenizedUrl.asFilterValuesFromResponse(data) //mapUrlTokensToFilterValues(tokenizedUrl, data)
