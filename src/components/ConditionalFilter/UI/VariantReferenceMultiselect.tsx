...
const VARIANT_REFERENCE_THUMBNAIL_SIZE = 12;

const VariantReferenceProductThumbnail = ({ url }: { url?: string }): JSX.Element => {
  const [failed, setFailed] = useState(false);

  if (!url || failed) {
    return (
      <span className={styles.thumbnailFallback} aria-hidden>
        <ImageOff size={VARIANT_REFERENCE_THUMBNAIL_SIZE} strokeWidth={iconStrokeWidth} />
      </span>
    );
  }
  ...
  return (
    <img
      ...
      data-test-id="variant-reference-thumbnail"
    />
  );
};
...
export const VariantReferenceMultiselect = ({
  ...
  return (
    <div className={styles.root} data-variant-reference="" onKeyDownCapture={handleKeyDownCapture}>
      <DynamicMultiselect
        {...inlineControlProps}
        width="100%"
        id={controlId}
        ...
        onChange={value => {
          emitter.changeRightOperator(index, value.map(toVariantReferencePill));
        }}
          ...
          {groups.map(group => (
            ...
            >
              <div className={styles.groupHeader}>
                <VariantReferenceProductThumbnail url={group.productThumbnailUrl} />
                <Text
                  size={2}
                  color="default2"
                  fontWeight="medium"
