...
export const TimelineEventHeader: React.FC<TimelineEventHeaderProps> = props => {
  ...
  return (
    <div className={classes.container}>
      ...
      {titleElements && (
        <div className={classes.elementsContainer}>
          {titleElements.filter(Boolean).map(({ text, link }) => {
            if (link) {
              return (
                <Link className={classes.titleElement} onClick={() => navigate(link)}>
                  {text}
                </Link>
              );
            }

            return <Typography className={classes.titleElement}>{text}</Typography>;
          })}
        </div>
      )}
      ...
      {secondaryTitle && <Typography className={classes.secondaryTitle}>{secondaryTitle}</Typography>}
    </div>
  );
};
