...
const ExitFormDialog: React.FC<ExitFormDialogProps> = ({ onLeave, onClose, isOpen }) => {
  ...
  return (
    <Dialog className={classes.container} open={isOpen} onClose={onClose}>
      <DialogHeader onClose={onClose}>{intl.formatMessage(messages.unableToSaveTitle)}</DialogHeader>
      <DialogContent className={classes.dialogContent}>
        <div className={classes.buttonsContainer}>
          ...
          <HorizontalSpacer />
          <Button variant="contained" color="primary" onClick={onLeave} data-test-id="ignore-changes">
