...
const checkEventPresence = (event: string) => {
  ...
  const excludedObjects = Object.keys(webhookTypes).filter(object => !availableObjects.includes(object));

  Object.keys(ExcludedDocumentMap).forEach(object => !excludedObjects.includes(object) && excludedObjects.push(object));
