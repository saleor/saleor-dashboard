...
import { capitalize, Dialog, DialogActions, DialogContent, DialogContentText, Typography } from '@material-ui/core';
import { Alert, Button, DialogHeader, List, ListBody, ListHeader, ListItem, ListItemCell } from '@saleor/macaw-ui';
...
const DryRun: React.FC<DryRunProps> = ({ setResult, showDialog, setShowDialog, query, syncEvents }: DryRunProps) => {
  const intl = useIntl();
            ...
            <List gridTemplate={['1fr 50px']}>
              <ListHeader>
                <ListItem className={classes.listHeader}>
                  <ListItemCell className={classes.listItemCell}>{intl.formatMessage(messages.objects)}</ListItemCell>
                  <ListItemCell></ListItemCell>
                </ListItem>
              </ListHeader>
              <ListBody className={classes.listBody}>
                {!availableObjects.length && <Typography>{intl.formatMessage(messages.noObjects)}</Typography>}
          ...
          <div className={classes.eventsWrapper}>
            {object ? (
              ...
              <>
                <ListHeader>
                  <ListItem className={classes.listHeader}>
                    <ListItemCell className={classes.listItemCell}>{intl.formatMessage(messages.item)}</ListItemCell>
                  </ListItem>
                </ListHeader>
                <ListBody className={classes.listBody}>
