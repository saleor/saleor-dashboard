...
import SingleAutocompleteSelectField, { SingleAutocompleteSelectFieldProps } from '../SingleAutocompleteSelectField';
import { Choice } from '../SingleSelectField';

export type DropdownChoice = Choice<string, string>;
export type DropdownCellContentProps = Pick<SingleAutocompleteSelectFieldProps, 'allowCustomValues' | 'emptyOption'>;
...
const DropdownCellEdit: ReturnType<ProvideEditorCallback<DropdownCell>> = ({ value: cell, onFinishedEditing }) => {
...
export const dropdownCellRenderer: CustomCellRenderer<DropdownCell> = {
  ...
  draw: (args, cell) => {
    ...
    ctx.fillText(value.label, rect.x + 8, rect.y + rect.height / 2 + getMiddleCenterBias(ctx, theme));
