...
const MoneyCellEdit: ReturnType<ProvideEditorCallback<MoneyCell>> = ({ value: cell, onChange: onChangeBase }) => {
...
export const moneyCellRenderer = (): CustomCellRenderer<MoneyCell> => ({
  ...
  draw: (args, cell) => {
    ...
    ctx.fillText(hasValue ? currency : '-', rect.x + 8, rect.y + rect.height / 2 + getMiddleCenterBias(ctx, theme));
    ctx.restore();
