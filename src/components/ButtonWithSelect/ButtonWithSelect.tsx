...
export interface ButtonWithSelectProps extends Omit<ButtonGroupProps, 'onClick'>, Pick<ButtonProps, 'onClick'> {
  options: Option[];
  href?: string;
}

export const ButtonWithSelect: React.FC<ButtonWithSelectProps> = ({ options, children, href, onClick, ...props }) => {
  ...
  return (
    <>
      <ButtonGroup ref={anchorRef} aria-label="button with select" {...props}>
        <Button variant="primary" color="primary" onClick={onClick} href={href} style={{ width: '100%' }}>
          {children}
        </Button>
        {options.length > 0 && (
          <Button
