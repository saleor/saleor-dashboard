...
/**
 * Pads a normalized price string to the currency's decimal places.
 * Use on blur for aligned display; leaves empty values unchanged.
 */
export const padPriceToDecimalPlaces = (value: string, maxDecimalPlaces: number): string => {
  ...
  const formatted = formatPriceInput(trimmed, maxDecimalPlaces);
  ...
  return parsed.toFixed(maxDecimalPlaces);
};
 ...
 */
export const sanitizeSpreadsheetPrice = (value: string, currency: string): string | null => {
  const trimmed = value.trim();
  ...
  const formatted = formatPriceInput(trimmed, getCurrencyDecimalPoints(currency));

  if (!formatted) {
    return null;
  }

  return padPriceToDecimalPlaces(formatted, getCurrencyDecimalPoints(currency));
};
