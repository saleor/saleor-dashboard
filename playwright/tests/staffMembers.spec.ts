...
test("TC: SALEOR_212 Edit a staff member @e2e @staff-members", async () => {
  ...
  await staffMembersPage.verifyAssignedPermission("Apps management");
  await staffMembersPage.verifyAssignedPermission("Channels management");
  await staffMembersPage.verifyAssignedPermission(USERS.staffToBeEdited.permissions[1]);
  await staffMembersPage.verifyAssignedPermission(USERS.staffToBeEdited.permissions[2]);
});
...
test("TC: SALEOR_215 Bulk delete staff members @e2e @staff-members", async () => {
  await config.goToConfigurationView();
  await config.openStaffMembers();
});
