...
test("TC: SALEOR_121 Should be able to add translation  @e2e @translations", async () => {
    ...
    await translationsPage.editTranslationNameButton.waitFor({ state: "visible", timeout: 50000});
    await translationsPage.page.getByText("Translation Category \"CategoryToTranslate\" - PL_PL").waitFor({ state: "visible", timeout: 50000});
...
test("TC: SALEOR_122 Should be able to edit translation  @e2e @translations", async () => {
    ...
    await translationsPage.editTranslationNameButton.waitFor({ state: "visible", timeout: 50000});
    await translationsPage.page.getByText("Translation Product \"Green Juice\" - PL_PL").waitFor({ state: "visible", timeout: 50000});
...
test("TC: SALEOR_123 Should be able to clear translation  @e2e @translations", async () => {
    ...
    await translationsPage.editTranslationNameButton.waitFor({ state: "visible", timeout: 50000});
    await translationsPage.page.getByText("Translation Collection \"Summer collection\" - PL_PL").waitFor({ state: "visible", timeout: 50000});
