...
test.describe.serial("TC: SALEOR_131", () => {
  for (const permission of permissionList) {
    const permissionTest = testWithPermission(permission, test);

    permissionTest(
      `User with ${permission} permissions should have readonly access to Apps @e2e @apps`,
      async ({ page }) => {
