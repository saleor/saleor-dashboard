...
test("TC: SALEOR_6 User should be able to navigate to discount list as a staff member using DISCOUNTS permission @basic-regression", async ({
  page,
}) => {
...
test("TC: SALEOR_7 User should be able to navigate to voucher list as a staff member using DISCOUNTS permission @basic-regression", async ({
  page,
}) => {
  const basePage = new BasePage(page);
  const mainMenuPage = new MainMenuPage(page);
