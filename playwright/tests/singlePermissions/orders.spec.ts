...
test("TC: SALEOR_8 User should be able to navigate to order list as a staff member using ORDER permission @basic-regression", async ({
  page,
}) => {
...
test("TC: SALEOR_9 User should be able to navigate to draft list as a staff member using ORDER permission @basic-regression", async ({
  page,
}) => {
  const basePage = new BasePage(page);
  const mainMenuPage = new MainMenuPage(page);
