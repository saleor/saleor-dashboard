...
test("TC: SALEOR_19 User should be able to navigate to staff members list page as a staff member using STAFF permission @basic-regression", async ({
  page,
}) => {
...
test("TC: SALEOR_20 User should be able to navigate to permission group list page as a staff member using STAFF permission @basic-regression", async ({
  page,
}) => {
  const mainMenuPage = new MainMenuPage(page);
  const basePage = new BasePage(page);
