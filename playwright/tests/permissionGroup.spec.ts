...
test("TC: SALEOR_132 Should be able to navigate to permission groups page @permissions @e2e", async () => {
...
test("TC: SALEOR_134 Should be able to edit existing permission group @permissions @e2e", async () => {
  ...
  await expect(permissionDetails.assignedMemberRow).not.toHaveText(
    assignedMembers.names[0],
  );
  await expect(permissionDetails.assignedMemberRow).toHaveText(
  ...
  await expect(permissionDetails.noMembersText).toBeVisible();
  await permissionDetails.clickSaveButton();
  await permissions.successBanner.waitFor({
    state: "visible",
    timeout: 50000,
